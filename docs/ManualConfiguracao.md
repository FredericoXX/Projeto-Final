...
O endpoint autenticado `POST /api/v1/retrieval/search` disponibiliza uma baseline lexical experimental sobre os chunks. Usa PostgreSQL Full-Text Search com configuração `simple`, coluna `TSVECTOR` gerada, índice GIN e ranking `ts_rank_cd`. A pesquisa fica limitada à instituição e idioma do utilizador, considera documentos ativos e válidos, seleciona a versão `processed` mais recente e aplica `official_only=true` por omissão.
