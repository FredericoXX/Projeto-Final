...
A variável `RETRIEVAL_MIN_RELEVANCE_SCORE` (padrão `0.05`) define o limiar mínimo de relevância lexical composta do retrieval do Momento 4. Numa pergunta com vários termos, candidatos com score abaixo do limiar são excluídos do `top_k`, exceto correspondências de frase exata; a dominância entre candidatos (um candidato cujos termos correspondidos são subconjunto próprio dos de um candidato melhor) também os remove. Consultas de um único termo institucional e o melhor candidato recuperado nunca são eliminados. O valor tem de ser finito e estar em `[0, 1]` (a aplicação recusa arrancar com valores inválidos); `0.0` desativa o piso, mantendo a dominância. **Apenas este limiar é configurável** — os pesos do ranking são constantes versionadas em `app/retrieval/reranking.py`, não variáveis de ambiente. Ver a secção de retrieval em [`docs/database.md`](database.md).
